...
public static class StreamExtensions
{
    ...
    public static async Task<long> SeekBackToNonZero(this Stream stream, long maxSeek, CancellationToken ct)
    {
        ...
        try
        {
            while (minPos < maxPos)
            {
                ...
                while (offset < toRead)
                {
                    if ((read = await stream.ReadAsync(buffer, offset, toRead - offset, ct).ConfigureAwait(false)) == 0)
    ...
    /// TODO: write docs
    public static async Task ReadCompletely(this Stream stream, Memory<byte> buffer, CancellationToken ct)
    {
        int read, offset = 0;
        while (offset < buffer.Length)
        {
            if ((read = await stream.ReadAsync(buffer.Slice(offset), ct).ConfigureAwait(false)) == 0)
                ThrowEndOfStreamException();
