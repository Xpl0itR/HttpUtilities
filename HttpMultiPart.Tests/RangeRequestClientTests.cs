...
public class RangeRequestClientTests : IDisposable
{
    ...
    public async Task TestRangeSupported() =>
        await RangeRequestClient.New(_httpClient, UrlSampleZipNormal, CancellationToken.None);

    [Fact]
    public async Task TestRangeNotSupported() =>
        await Assert.ThrowsAsync<NotSupportedException>(() => RangeRequestClient.New(_httpClient, UrlSampleZipLegacy, CancellationToken.None));
    ...
    {
        RangeRequestClient reqClient = await RangeRequestClient.New(_httpClient, UrlSampleZipNormal, CancellationToken.None);
